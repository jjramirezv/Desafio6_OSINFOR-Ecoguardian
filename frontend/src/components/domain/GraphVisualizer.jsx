...
function layoutGraph(nodes, edges) {
  ...
  const midY = maxLevel * (NODE_H + LEVEL_GAP) / 2;
  ...
  return { positioned, svgEdges, svgW, svgH, midY, maxLevel };
}

function NodeShape({ node, typeClass }) {
  const cx = node.x;
  const cy = node.y;
      ...
      >
        {layout.svgEdges.map((e) => (
          <g key={e.id || `${e.source}-${e.target}`}>
            <path d={e.path} className="graph-edge" />
            <polygon
        ...
        {layout.positioned.map((node) => (
          <NodeShape
            key={node.id}
            node={node}
            typeClass={nodeTypeClass(node.type || node.node_type)}
          />
        ))}
      </svg>
    </div>
